...
def find_edge_type(args, I, Map_background, Map_edge, Map_surface):
    ...
    for i in range(I.shape[0]):
        for j in range(I.shape[1]):
            if Map_edge[i, j] == 1: 
                below = Map_background[i, j - 1] if j - 1 >= 0 else 0
                if below == 1:
                    Map_contact_edge[i, j] = 1
    ...
    for i in range(I.shape[0]):
        for j in range(I.shape[1]):
            if Map_edge[i, j] == 1:
                if edge_angle[i, j] < np.pi/4:  # closer to 0 indicates horizontal
                    Map_vertical_edge[i, j] = 1
                else:  # closer to pi/2 indicates vertical
                    Map_horizontal_edge[i, j] = 1
...
def Y_3D(I, Map_background, Map_horizontal_edge, Map_vertical_edge, Map_contact_edge, Map_surface, theta):
    ...
    for i in range(I.shape[0]):
        for j in range(1, I.shape[1]):
            if Map_vertical_edge[i, j] == 1:
                temp = np.zeros((1, I.size))
                temp[0, i + j * I.shape[0]] = 1
                temp[0, i + (j - 1) * I.shape[0]] = -1
                A = np.concatenate((A, temp), axis=0)
                b = np.concatenate((b, np.ones((1, 1)) * (1 / (np.cos(theta) + 1e-8))), axis=0)

    # Horizontal edges
    for i in range(1, I.shape[0]):
        for j in range(I.shape[1]):
            if Map_horizontal_edge[i, j] == 1:
                temp = np.zeros((1, I.size))
                temp[0, i + j * I.shape[0]] = 1
                temp[0, i + (j - 1) * I.shape[0]] = -1
                A = np.concatenate((A, temp), axis=0)
    ...
    for i in range(1, I.shape[0] - 1):
        for j in range(1, I.shape[1]-1):
            if Map_surface[i, j] == 1:
                temp = np.zeros((1, I.size))
                temp[0, i + j * I.shape[0]] = 2
                temp[0, (i + 1) + j * I.shape[0]] = -1
                temp[0, (i - 1) + j * I.shape[0]] = -1
                A = np.concatenate((A, temp), axis=0)
                b = np.concatenate((b, np.zeros((1, 1))), axis=0)
    ...
    Y = np.matmul(
        np.linalg.inv(np.matmul(A.transpose(), A) + 0.0000001 * np.identity(A.shape[1])),
...
def Z_3D(I, yy, Y, theta):
    ...
    #### Your job 5 starts here: generate Z (<10 lines) ####

    Z = (np.cos(theta) * Y - yy) / (np.sin(theta) + 1e-8)

    #### Your job 5 ends here: generate Z ####
